...
# 参数
param = utils.param
root_path = './../data/entity_link'
# 生成 mention 的候选实体字典
if os.path.exists(os.path.join(root_path, 'generated/cand.json')):
    with open(os.path.join(root_path, 'generated/cand.json'), 'r', encoding='utf8') as f:
        jsonstr = ''.join(f.readlines())
        cand_dic = json.loads(jsonstr)
    with open(os.path.join(root_path, 'generated/entity.json'), 'r', encoding='utf8') as f:
        jsonstr = ''.join(f.readlines())
...
# 生成训练、验证、测试的文本数据
if not os.path.exists(os.path.join(root_path, 'generated/train_data.txt')):
    GeneratePairwaiseSample('train.json', cand_dic, ent_dic, is_train=True)
if not os.path.exists(os.path.join(root_path, 'generated/dev_data.txt')):
    GeneratePairwaiseSample('dev.json', cand_dic, ent_dic, is_train=False)
if not os.path.exists(os.path.join(root_path, 'generated/test_data.txt')):
...
if not os.path.exists(os.path.join(root_path, 'pretrain_data/matrix.npy')):
    matrix, vocab, vec_dim, vocab_size = utils.loadWord2Vec("../data/pretrain_data/word2vec.iter5")
else:
    matrix = np.load(os.path.join(root_path, 'pretrain_data/matrix.npy'))
    with open(os.path.join(root_path, 'pretrain_data/vocab.json'), 'r', encoding='utf8') as f:
        jsonstr = ''.join(f.readlines())
...
if not os.path.exists(os.path.join(root_path, 'generated/train.csv')):
    data_encoder.data_encode(os.path.join(root_path, "generated/train_data.txt"), is_train=True)
if not os.path.exists(os.path.join(root_path, 'generated/dev.csv')):
    data_encoder.data_encode(os.path.join(root_path, "generated/dev_data.txt"), is_train=False)
if not os.path.exists(os.path.join(root_path, 'generated/test.csv')):
    data_encoder.data_encode(os.path.join(root_path, "generated/test_data.txt"), is_train=False)

# 构建数据集加载接口
train_set = DataSet(os.path.join(root_path, "generated/train_part.csv"), is_train=True)
dev_set = DataSet(os.path.join(root_path, "generated/dev.csv"), is_train=False)
test_set = DataSet(os.path.join(root_path, "generated/test.csv"), is_train=False)
...
for epoch in range(param["epoch"]):
    ...
    if not os.path.exists("../../ckpt/entity_link"):
        os.mkdir("../../ckpt/entity_link")
    torch.save(checkpoint, '../../ckpt/entity_link/ckpt_best_%s.pth' % (str(epoch)))
