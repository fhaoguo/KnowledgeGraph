...
import json
import numpy as np
main_path = '../data/entity_link'
# matrix 向量数组；vocab 包含 vocab["w2i"]: word2idx、vocab["i2w"]：idx2word；向量维度，字词数
matrix = np.load(os.path.join(main_path, 'pretrain_data/matrix.npy'))
with open(os.path.join(main_path, 'pretrain_data/vocab.json'), 'r', encoding='utf8') as f:
    jsonstr = ''.join(f.readlines())
    vocab = json.loads(jsonstr)

# 生成 mention 的候选实体字典
if os.path.exists(os.path.join(main_path, 'generated/cand.json')):
    with open(os.path.join(main_path, 'generated/cand.json'), 'r', encoding='utf8') as f:
        jsonstr = ''.join(f.readlines())
        cand_dic = json.loads(jsonstr)
    with open(os.path.join(main_path, 'generated/entity.json'), 'r', encoding='utf8') as f:
        jsonstr = ''.join(f.readlines())
...
def predict(sentence):
    ...
    with open(os.path.join(main_path, "basic_data/predict.json"), 'w', encoding='utf-8') as jsonfile:
        jsonfile.write(jsonstr)
    # 生成预测的文本数据
    GeneratePairwaiseSample('predict.json', cand_dic, ent_dic, is_train=False)
    # 数据编码
    data_encoder.data_encode(os.path.join(main_path, "generated/predict_data.txt"),
                             is_train=False)
    # 构建数据集加载接口
    predict_set = DataSet(os.path.join(main_path, "generated/predict.csv"), is_train=False)
    # dataloader
    ...
    # 处理预测结果，生成打印信息
    data = []
    with open(os.path.join(main_path, 'basic_data/predict.json'), 'r', encoding='utf8') as f:
    ...
    # 打印结果
    f = open(os.path.join(main_path, 'basic_data/predict_result.json'), 'w+', encoding='utf8')
    for i in data:
        print(i['text'])
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        sentence = input('>>>  ')
        if sentence.strip() in ['quit', 'exit', 'q', 'e']:
            break
        predict(sentence)
